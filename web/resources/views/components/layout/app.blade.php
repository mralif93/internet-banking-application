@props([
    'title' => 'CentraBank Direct - Digital Retail Banking',
    'showSidebar' => false,
])
...
<head>
    ...
    <meta name="description" content="Next-Generation Personal Retail Digital Banking with DuitNow instant transfers and biometric security.">
    <title>{{ $title }}</title>

    <!-- Pre-render theme check to prevent flash of wrong theme (FOUC) -->
    <script>
        (function() {
            try {
    ...
    <!-- Animate.css for rich interactive micro-animations -->
...
<body class="h-full bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-sans antialiased transition-colors duration-200 flex flex-col min-h-screen selection:bg-emerald-500 selection:text-white">
    ...
    <!-- Main Content Container (Full Width Balanced Landing Layout) -->
    ...
    <!-- Regulatory Banking Footer (Compact sm) -->
            ...
            <div class="flex flex-col sm:flex-row items-center justify-between gap-2.5 text-center sm:text-left">
                ...
                <div class="flex items-center gap-2.5 text-[10px] flex-wrap justify-center">
                    <span class="inline-flex items-center gap-1 text-emerald-600 dark:text-emerald-400">
                        ...
                        TLS 1.3 / PayNet Direct Connected
                    </span>
                    <span class="text-slate-300 dark:text-slate-700">&bull;</span>
                    <x-banking.security-badge type="kill_switch" />
                    <span class="text-slate-300 dark:text-slate-700">&bull;</span>
                    <span class="text-slate-400 dark:text-slate-500">&copy; 2026 CentraBank Direct</span>
                </div>
            </div>
        </div>
    </footer>
